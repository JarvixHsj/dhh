...
class NodeService extends CommonService {
    ...
    public function mergeNodes($node, $pid = 0){
        $arr = array();
        foreach ($node as $v) {
            if ($v["pid"] == $pid) {
                $v["children"] = $this->mergeNodes($node, $v["id"]);
                $arr[] = array_filter($v);
            }
        }
        return $arr;
     ...
     * 设置节点菜单项
     * @param  int   $id     节点id
     * @param  int   $menu 节点菜单项
     * @return mixed
     */
    public function setMenu($id, $menu) {
        $result = $this->getM()
                    ->where("id={$id}")
                    ->save(array('menu' => $menu));
    ...
    public function delete($id) {
        ...
        $node = $this->getM()->getById($id);
        if (empty($node)) {
            return $this->errorReturn('您需要删除的节点不存在！');
        }
    ...
    protected function getNodeLevelByPid($pid) {
        $node = $this->getM()->getById($pid);
        if (empty($node)) {
            return -1;
        }
     ...
     * @return array
     */
    public function getMenuNodes(array $where) {
        if (!isset($where) || !is_array($where)) {
            $where = array();
        }

        $map = array('status' => 1, 'menu' => 1);
        return $this->getM()->where(array_merge($map, $where))->select();
    }
     ...
     * @return array
     */
    private function replaceMenuItem() {
        $nodes = $this->getMenuNodes();
        $menu_nodes = $this->mergeNodes($nodes, 1);

        $menu = array();
        foreach ($menu_nodes as $key => $value) {
            $ctrlName = $value['name'];
            $menu[$ctrlName] = array(
                'name' => $value['title'],
            ...
            foreach ($value['children'] as $child_key => $child_value) {
                $url = "{$ctrlName}/" . $child_value['name'];
                $sub_menu[] = array('item' => array("{$url}" => $child_value['title']));
            }
            $menu[$ctrlName]['sub_menu'] = $sub_menu;
        }

        $modelLogic = D('Model', 'Logic');
        return $modelLogic->writeMenu($menu);
    }
}
