...
<script type="text/javascript">
    ...
    var cattleDataRow = {};
    cattleDataRow = {};
    <?php if(is_array($cattle)): foreach($cattle as $key=>$vo): ?>cattleDataRow = {};
    cattleDataRow.id = <?php echo ($vo["id"]); ?>;
    cattleDataRow.text = '<?php echo ($vo["name"]); ?>';
    cattleDatas.push(cattleDataRow);<?php endforeach; endif; ?>

    $(function ()
    {
        var proData = proDatas;
        var cityData = cityDatas;
        var cattle = cattleDatas;
        ...
        $("#cattle").ligerComboBox({
            data: cattle, isMultiSelect: false,
            onSelected: function (newvalue)
            {
                var newData = new Array();
                for (i = 0; i < cityData.length; i++)
...
<form action="<?php echo U(CONTROLLER_NAME.'/create');?>" method="post">
    <table>
        <tr>
            <td>黄牛名称：</td>
            <td><input type="text" name="cattle_id" id="cattle"/></td>
        </tr>
        <tr>
            <td>起始地：</td>
